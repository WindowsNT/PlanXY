...
#include "ResultsKids.g.h"
ystring trim(ystring s);

namespace winrt::WuiFET::implementation
{
    struct ResultsKids : ResultsKidsT<ResultsKids>
    {
        ResultsKids()
        {
        ...
        // WhatX 1: Kid
        winrt::Windows::Foundation::Collections::IObservableVector<winrt::WuiFET::Item> Kids_List()
        {
            return GetList((_ResultP->x->GetRootElement()["Kids_List"]));
        }
        XML3::XMLElement* _SelectedKid = 0;
        void KidChanged(IInspectable, IInspectable)
        {
            auto lv = Kids_ListView();
            auto sel = lv.SelectedItem().as<winrt::WuiFET::Item>();
            if (sel && sel.ptr())
                _SelectedKid = (XML3::XMLElement*)sel.ptr();
            Refresh(L"gr1_is");
        }
        ...
        winrt::Windows::Foundation::Collections::IObservableVector<DGRowModel> gr1_is()
        {
            ...
            ystring SelectedKidName = trim(_SelectedKid->FindElementZ("Name", true)->GetContent());
            if (!_ResultP->x)
                return rows;
            auto x = _ResultP->x;
            XML3::XMLElement* root = &x->GetRootElement();
...
{
    struct ResultsKids : ResultsKidsT<ResultsKids, implementation::ResultsKids>
    {
    };
}
