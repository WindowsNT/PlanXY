...
namespace winrt::WuiFET::implementation
{
    ...
    DependencyProperty Resizer::m_orientationProperty =
        DependencyProperty::Register(
            L"Orientation",
            xaml_typename<WuiFET::SplitOrientation>(),
            xaml_typename<WuiFET::Resizer>(),
            PropertyMetadata{ box_value(WuiFET::SplitOrientation::Vertical) }
        );

    Resizer::Resizer()
    {
        DefaultStyleKey(winrt::box_value(L"WuiFET.Resizer"));
    }
    ...
    void Resizer::OnApplyTemplate()
    {
        ...
        if (Orientation() == WuiFET::SplitOrientation::Horizontal)
    ...
    WuiFET::SplitOrientation Resizer::Orientation() const
    {
        return winrt::unbox_value<WuiFET::SplitOrientation>(GetValue(m_orientationProperty));
    }

    void Resizer::Orientation(WuiFET::SplitOrientation const& value)
    {
    ...
    {
        if (auto control = d.try_as<WuiFET::Resizer>())
        {
    ...
    winrt::event_token Resizer::ValueChanged(TypedEventHandler<WuiFET::Resizer, IInspectable> const& handler)
    {
