...
void dd2Click(IInspectable const& sender, Microsoft::UI::Xaml::RoutedEventArgs)
{
    if (!_SelectedClass)
        return;
    ...
    for (auto& r1 : *r0)
    {
        if (r1.GetElementName() != "ConstraintStudentsSetNotAvailableTimes")
            continue;
        // This Class?
        auto tn = trim(r1.FindElementZ("Students", true)->GetContent());
        if (tn != SelectedClassName)
            continue;
        ystring y;
        yy++;
        ...
        item.Click([&](IInspectable const& sender, RoutedEventArgs const&)
            {
                ...
                _SelectedClassEntry = (XML3::XMLElement*)unbox_value<long long>(it.Tag());
                _gr2_p = _wtoi(trim(_SelectedClassEntry->FindElementZ("Weight_Percentage", true)->GetContent()).c_str());
                Refresh(L"gr2_p");
                Refresh(L"gr2_is");
                Refresh(L"gr2_ch");
...
winrt::Windows::Foundation::Collections::IObservableVector<winrt::WuiFET::DGHeaderModel> gr2_ch()
{
...
void gr2_p(int value)
{
    if (_gr2_p != value)
    {
        _gr2_p = value;
        m_propertyChanged(*this, winrt::Microsoft::UI::Xaml::Data::PropertyChangedEventArgs(L"gr2_p"));

        if (_SelectedClassEntry)
        {
            ystring v;
            v.Format(L"%i", value);
            _SelectedClassEntry->FindElementZ("Weight_Percentage", true)->SetContent(v);
        }
    }
}
std::wstring _gr2_n1;
winrt::hstring gr2_n1()
{
    return _gr2_n1.c_str();
}
void gr2_n1(winrt::hstring value)
{
    if (_gr2_n1 != value)
    {
        _gr2_n1 = value;
...
winrt::Windows::Foundation::Collections::IObservableVector<DGRowModel> gr2_is()
{
    ...
    for (auto& hou : *hours)
    {
        ...
        for (auto& day : *days)
        {
			...
			if (_SelectedClassEntry) // ConstraintStudentsetNotAvailableTimes
            {
                bool Put = 0;
                c1.Ptr4((long long)_SelectedClassEntry);
                for (auto& el : *_SelectedClassEntry)
                {
            ...
            c1.Ptr3((long long)_SelectedClass); // Class
